...
contract maincontract{
    ...
    function addNewContributor(uint256 newContributor, uint256 projectId, uint256 _share) public {
        require(newContributor>0 &&  newContributor<=totalUserCount, "No such user exist!");
        require(_share > 0, "A contributor must have shares");
        ...
        projects[projectId-1].shares[members-1] = _share;                            //shares
    ...
    function allocateSharesTo(uint256 Newshares, uint256 projectId, uint256 _contributorId) public {
        require(Newshares>0, "A contributor must have shares");
